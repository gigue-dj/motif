//! `morceau-wasm` exposes `morceau-core` to host languages via
...
//!
//! - [`Morceau::open`] takes the same TOML config as the native API,
...
//! - [`Morceau::open_with_host_storage`] (v0.0.3-alpha.3) takes the same
//!   TOML config plus a host-supplied JS object implementing the
//!   [`MorceauHostStorage`] interface (OPFS, app sandbox, RN bridge,
...
//! - [`Morceau::query`] takes a Cypher string and a JSON-encoded params
...
use morceau_core::{Engine, InMemoryController, InMemoryHandle, MorceauConfig, Params, Value};
use wasm_bindgen::prelude::*;

pub use host_storage::{MorceauHostStorage, WasmHostStorage};
...
pub fn validate_config(toml_src: &str) -> Result<(), JsError> {
    MorceauConfig::from_toml_str(toml_src)
        .map(|_| ())
...
#[wasm_bindgen]
impl Morceau {
    /// Open an in-memory Morceau instance configured by `toml_src`.
    /// `storage.path` from the config is ignored — for persistence
    /// across page loads / app restarts on wasm, use
    /// [`Morceau::open_with_host_storage`] with an OPFS / app-sandbox /
    /// RN-bridge backed JS object.
    #[wasm_bindgen(constructor)]
    pub fn open(toml_src: &str) -> Result<Morceau, JsError> {
        let cfg = MorceauConfig::from_toml_str(toml_src).map_err(to_js)?;
    ...
    /// Open Morceau with a host-supplied [`MorceauHostStorage`] backend.
    ...
    pub fn open_with_host_storage(
        toml_src: &str,
        storage: MorceauHostStorage,
    ) -> Result<Morceau, JsError> {
        let cfg = MorceauConfig::from_toml_str(toml_src).map_err(to_js)?;
...
fn parse_params(json: &str) -> Result<Params, JsError> {
    ...
    for (k, v) in map {
        out.insert(k.clone(), json_value_to_morceau_value(v)?);
    }
    Ok(out)
}

fn json_value_to_morceau_value(v: &serde_json::Value) -> Result<Value, JsError> {
