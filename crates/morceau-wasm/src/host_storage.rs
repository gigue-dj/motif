...
//!   /// answer (morceau treats `undefined` as "unknown" and skips
...
//! exact up to 2^53 (~9 PB) — well past the size of any morceau store
...
//! morceau-wasm to a host platform; the trait + interface keeps morceau
//! controller-agnostic the same way the `Controller` trait keeps it
//! controller-transport-agnostic.

use morceau_core::{Storage, StorageError};
...
extern "C" {
    ...
    /// `Morceau::open_with_host_storage`. Morceau holds the object for
    /// the lifetime of the engine and calls into it on every
    /// append / read / truncate.
    #[wasm_bindgen(typescript_type = "MorceauHostStorage")]
    pub type MorceauHostStorage;

    #[wasm_bindgen(method, structural, catch)]
    fn append(this: &MorceauHostStorage, bytes: &[u8]) -> Result<f64, JsValue>;

    #[wasm_bindgen(method, structural, catch, js_name = readAt)]
    fn read_at(
        this: &MorceauHostStorage,
        offset: f64,
        len: u32,
    ) -> Result<js_sys::Uint8Array, JsValue>;

    #[wasm_bindgen(method, structural)]
    fn len(this: &MorceauHostStorage) -> f64;

    #[wasm_bindgen(method, structural, catch)]
    fn truncate(this: &MorceauHostStorage, new_len: f64) -> Result<(), JsValue>;

    #[wasm_bindgen(method, structural, js_name = freeSpace)]
    fn free_space(this: &MorceauHostStorage) -> JsValue;
}

/// Rust wrapper around a host-supplied [`MorceauHostStorage`] JS object.
...
/// [`morceau_core::storage::MaybeSend`] marker.
pub struct WasmHostStorage {
    inner: MorceauHostStorage,
}

impl WasmHostStorage {
    pub fn new(inner: MorceauHostStorage) -> Self {
        Self { inner }
    }
}
