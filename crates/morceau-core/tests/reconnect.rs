...
use morceau_core::{
    Controller, ControllerConfig, ControllerError, EdgeConfig, Engine, IdentityConfig,
    MorceauConfig, Mutation, Node, StorageConfig,
};
...
impl Controller for FlakyController {
    fn connect(
        &mut self,
        _capability: &morceau_core::CapabilityConfig,
...
fn worker_retries_transient_failures() {
    ...
    match &received[0].op {
        morceau_core::MutationOp::NodeInsert(n) => assert_eq!(n.id, "eventually-lands"),
