...
//! `_morceau.foreshadow` Cypher metadata namespace (MORCEAU.md decisions
//! 19, 20, 22).
//!
...
//! preserving the foreshadow state; and Cypher `n._morceau.foreshadow`
//! returns the right answer in both `WHERE` and `RETURN` positions.

use std::path::PathBuf;

use morceau_core::{
    ControllerConfig, Engine, IdentityConfig, MorceauConfig, Node, Params, ResultCell,
    StorageConfig, Value,
};
...
#[test]
fn morceau_foreshadow_in_where_clause() {
    ...
    let r = e
        .query(
            "MATCH (n) WHERE n._morceau.foreshadow = true RETURN n",
            &Params::new(),
        )
        .unwrap();
    ...
    let r = e
        .query(
            "MATCH (n) WHERE n._morceau.foreshadow = false RETURN n",
            &Params::new(),
        )
        .unwrap();
...
#[test]
fn morceau_foreshadow_in_return_projection() {
    let cfg = config_with(PathBuf::from(":memory:"));
    let mut e = Engine::open_in_memory(&cfg).unwrap();
    e.query("CREATE (n:Person {id: 'a'})", &Params::new())
        .unwrap();
    ...
    let r = e
        .query("MATCH (n) RETURN n._morceau.foreshadow", &Params::new())
        .unwrap();
    assert_eq!(r.columns, vec!["n._morceau.foreshadow"]);
    assert_eq!(r.rows.len(), 1);
    assert_eq!(r.rows[0][0], ResultCell::Value(Value::Bool(true)));
}

#[test]
fn unknown_morceau_keys_resolve_to_null() {
    let cfg = config_with(PathBuf::from(":memory:"));
    let mut e = Engine::open_in_memory(&cfg).unwrap();
    e.query("CREATE (n:Person {id: 'a'})", &Params::new())
        .unwrap();

    // Forward-compatible probe: `_morceau.something_added_in_v0_0_3` is
    // valid syntax and resolves to NULL for now.
    let r = e
        .query("MATCH (n) RETURN n._morceau.future_thing", &Params::new())
        .unwrap();
    assert_eq!(r.rows.len(), 1);
...
fn v0_0_1_store_is_rejected_on_open() {
    ...
    // by design.)
    let dir = TempDir::new().unwrap();
    let path = dir.path().join("legacy.morceau");

    // Forge a v0.0.1 header: MAGIC + version=1 + zero pad to 16 bytes.
    // Uses the post-rename MAGIC (`b"MORCEAU\x01"`); pre-rename `b"MOTIF\0\0\x01"`
    // stores trip the BadMagic path before they reach the version check.
    let mut hdr = Vec::new();
    hdr.extend_from_slice(b"MORCEAU\x01"); // MAGIC
