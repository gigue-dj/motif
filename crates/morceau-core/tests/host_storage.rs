...
//!
//! The actual `WasmHostStorage` lives in `morceau-wasm` and bridges to a
...
//! unknown -p morceau-wasm` (which builds `WasmHostStorage` against the
...
use morceau_core::storage::{Storage, StorageError};
use morceau_core::{
    ControllerConfig, Engine, IdentityConfig, MorceauConfig, Node, StorageConfig, Value,
};

/// Mirrors `morceau_core::storage::HEADER_LEN` (pub(crate) inside
/// morceau-core). Bump if the on-disk header layout grows.
...
impl RustHostStorage {
    fn new() -> Self {
        ...
        bytes.extend_from_slice(b"MORCEAU\x01");
