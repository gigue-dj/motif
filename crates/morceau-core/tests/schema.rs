...
//!
//! Covers MORCEAU.md alpha.3 commitments:
...
//! - `_morceau.schema.version` resolves to the current version via the
//!   metadata-as-data Cypher namespace.

use std::path::PathBuf;

use morceau_core::{
    ControllerConfig, Engine, EngineError, IdentityConfig, MorceauConfig, Node, Params,
    PropertyType, ResultCell, Schema, StorageConfig, TableKind, TableSchema, Value,
};
...
#[test]
fn morceau_schema_version_resolves_via_cypher() {
    let cfg = config_with(PathBuf::from(":memory:"));
    let mut e = Engine::open_in_memory(&cfg).unwrap();
    ...
    let r = e
        .query("MATCH (n) RETURN n._morceau.schema.version", &Params::new())
        .unwrap();
    assert_eq!(r.columns, vec!["n._morceau.schema.version"]);
    assert_eq!(r.rows.len(), 1);
    assert_eq!(r.rows[0][0], ResultCell::Value(Value::I64(7)));
}

#[test]
fn morceau_schema_version_is_null_when_no_schema() {
    let cfg = config_with(PathBuf::from(":memory:"));
    let mut e = Engine::open_in_memory(&cfg).unwrap();
    e.query("CREATE (n:Person {id: 'a'})", &Params::new())
        .unwrap();
