...
//! "exit criterion 2" test (`Morceau::open(&MorceauConfig)` works end-to-end)
//! while the wasm-bindgen layer is still pending.

use std::path::PathBuf;

use morceau_core::{
    ControllerConfig, Edge, Engine, IdentityConfig, MorceauConfig, Node, StorageConfig, Value,
};
...
fn durable_append_recovers_across_reopen() {
    ...
    let path = dir.path().join("morceau.db");
