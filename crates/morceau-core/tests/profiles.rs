//! Test profiles per MORCEAU.md decision 22. v0.0.2-alpha.4 stands up
...
use morceau_core::{
    ControllerConfig, Engine, IdentityConfig, InMemoryController, MorceauConfig, Node, Storage,
...
fn config_for_tests(path: PathBuf) -> MorceauConfig {
    MorceauConfig {
...
fn burst_default_profile() {
    run_burst_with_storage(
        Box::new(morceau_core::MemoryStorage::new()),
        30,
...
fn burst_potato_profile() {
    let inner = morceau_core::MemoryStorage::new();
