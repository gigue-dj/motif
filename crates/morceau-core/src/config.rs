//! TOML-backed configuration for Morceau. The host application owns the
//! `morceau.toml` file; Morceau treats it as read-only at open time. See
//! `morceau.toml.example` at the repo root for the canonical schema.
...
/// Top-level Morceau configuration. Loaded from a `morceau.toml` file (or
/// constructed in code for tests).
#[derive(Debug, Clone, Deserialize, Serialize, PartialEq, Eq)]
pub struct MorceauConfig {
    pub identity: IdentityConfig,
    pub controller: ControllerConfig,
    pub storage: StorageConfig,
    /// Deterministic facts about the host hardware (RAM MB, CPU cores,
    /// arch, etc.). Morceau reports facts; controller decides policy.
...
/// resolution. Morceau compares opaque tokens; the host owns the auth flow
...
/// `ControllerKind` enum: morceau-core is controller-agnostic
/// (MORCEAU.md decision 18) so a finite enum here would have to enumerate
/// every bridge, which violates the OSS posture. `kind` is now an
/// opaque string that the host's bridge crate (or `morceau-cli` /
/// `morceau-wasm`) interprets.
///
/// Conventional values:
/// - `"in-memory"` — the default `InMemoryController` shipped in
///   morceau-core; useful for tests, local development, and the alpha.2
///   threading-model validation.
/// - `"external"` — host wires its own [`crate::sync::Controller`]
///   programmatically; morceau-core does not auto-instantiate one.
/// - `"<bridge-name>"` — interpreted by the corresponding bridge crate
///   (e.g. `morceau-surreal-bridge` looks for `kind = "surreal"`).
#[derive(Debug, Clone, Deserialize, Serialize, PartialEq, Eq)]
pub struct ControllerConfig {
    pub kind: String,
}

/// Where Morceau's local single-file store lives.
#[derive(Debug, Clone, Deserialize, Serialize, PartialEq, Eq)]
pub struct StorageConfig {
    pub path: PathBuf,
}

/// Deterministic facts about the host hardware. Morceau reports these
/// to the controller (via the bridge) so the controller can choose
/// between `edge-is-tiny` (cache + foreshadow) and `edge-is-free`
/// (local execution) routing strategies. Per MORCEAU.md decision 20,
...
pub struct EdgeConfig {
    ...
    /// `"push"` — controller pushes schema; morceau caches the latest.
    /// `"fetch"` — morceau fetches schema lazily on first reference.
...
impl MorceauConfig {
    ...
    pub fn to_toml_string(&self) -> String {
        toml::to_string(self).expect("MorceauConfig is always serializable")
    }
}
...
mod tests {
    ...
    fn round_trips() {
        ...
        let reparsed = MorceauConfig::from_toml_str(&emitted).unwrap();
        assert_eq!(cfg, reparsed);
    }

    #[test]
    fn parses_arbitrary_kind() {
        // morceau-core itself doesn't validate `kind`; that's the bridge's
        ...
        let src = r#"
            ...
            path = "./x.db"
        "#;
        let cfg = MorceauConfig::from_toml_str(src).unwrap();
    ...
    fn missing_field_is_an_error() {
        let bad = r#"
            ...
            path = "./morceau.db"
        "#;
        let err = MorceauConfig::from_toml_str(bad).unwrap_err();
