...
//!
//! - `Engine::open(&MorceauConfig)` — file-backed, replays the log on open.
//! - `Engine::open_with(&MorceauConfig, Box<dyn Storage>)` — pluggable.
...
//!   `_morceau.foreshadow` and other metadata-as-data namespaces are
...
use crate::config::{CapabilityConfig, EdgeConfig, MorceauConfig};
...
pub struct Engine {
    ...
    /// Per-instance actor identity copied from `MorceauConfig::identity`.
...
impl Engine {
    ...
    pub fn open(config: &MorceauConfig) -> Result<Self, EngineError> {
    ...
    pub fn open_in_memory(config: &MorceauConfig) -> Result<Self, EngineError> {
    ...
    /// `morceau.toml`'s `[controller]` section. Catches silent typos —
    /// the kind is opaque to morceau-core (so any string parses), but
    ...
    /// declaration in `morceau.toml`'s `[capability]` overlaid on the
    ...
    ///
    /// Exposed via the `_morceau.foreshadow` Cypher metadata namespace
    /// per MORCEAU.md decision 19 (metadata-as-data).
