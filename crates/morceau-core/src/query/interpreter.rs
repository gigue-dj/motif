...
//! index.
//!
//! `_morceau.<key>` paths in `WHERE` / `RETURN` resolve against
//! engine state rather than on-disk properties (MORCEAU.md decision 19;
//! `_morceau.foreshadow` → live foreshadow flag, `_morceau.schema.version`
...
/// hit the node's user properties; `_morceau.<rest>` paths hit the
/// metadata-as-data namespace (engine state). Arbitrary-depth paths
/// are allowed only inside the `_morceau` namespace; user-property
...
fn resolve_node_property_path(
    ...
    match path {
        ...
        [namespace, rest @ ..] if namespace == "_morceau" => {
            Ok(morceau_metadata(node, rest, engine))
        }
...
/// participate in the `_morceau` namespace — the foreshadow flag is
/// engine-wide and reachable via the node-bound path; future alphas
/// may expose `r._morceau.X` if a use case appears.
...
/// `_morceau.<...>` lookups. Unknown keys return `Value::Null` rather
/// than an error so hosts can probe the namespace forward-compatibly.
fn morceau_metadata(node: &Node, path: &[String], engine: &Engine) -> Value {
    match path {
