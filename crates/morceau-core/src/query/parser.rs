...
mod tests {
    ...
    #[test]
    fn parses_morceau_metadata_path() {
        let s = parse("MATCH (n) WHERE n._morceau.foreshadow = true RETURN n").unwrap();
        ...
        if let Expr::Binary { lhs, .. } = where_clause {
            if let Expr::Property { variable, path } = *lhs {
                ...
                assert_eq!(path, vec!["_morceau".to_string(), "foreshadow".to_string()]);
                return;
            }
        }
    ...
    fn parses_metadata_in_return() {
        let s = parse("MATCH (n) RETURN n._morceau.foreshadow").unwrap();
        ...
        assert_eq!(
            ...
            vec![ReturnItem::Property {
                variable: "n".into(),
                path: vec!["_morceau".into(), "foreshadow".into()],
            }]
        );
    }
}
