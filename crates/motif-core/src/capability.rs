...
//!
//! Per MOTIF.md decision 20, motif reports **deterministic facts** about
...
//! as enough.
//!
//! v0.0.2 stored a host-supplied [`CapabilityConfig`] from `motif.toml`
...
//! always motif's view — they may not be colocated or allocated the
//! same budget) and treats `motif.toml`'s `[capability]` as a
//! field-level **override**:
//!
//! - If the host declared `cpu_cores = 4` in TOML, that wins.
//! - If a field is absent, motif probes — `std::thread::available_parallelism`
