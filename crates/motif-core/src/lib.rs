...
//!
//! - Configuration loaded from `motif.toml` ([`MotifConfig`],
//!   [`config`]).
...
//!   own backend via `motif-wasm`'s `WasmHostStorage`.
...
//!   `DELETE` / `DETACH DELETE`. Plus the `_motif.X` metadata-as-
//!   data namespace (per `MOTIF.md` decision 19).
...
//! freezes with semver promises (see `MOTIF.md` long-run strategy).
//!
//! See `MOTIF.md` for the milestone plan and `LIMITATIONS.md` for
