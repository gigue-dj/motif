...
//!   wired the controller it declared in `MotifConfig::controller.kind`.
...
use motif_core::{
    ControllerConfig, Engine, EngineError, IdentityConfig, InMemoryController, MotifConfig, Node,
    StorageConfig,
};
...
fn config_with(path: PathBuf, kind: &str) -> MotifConfig {
    MotifConfig {
