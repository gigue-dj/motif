# morceau-db — design notes and roadmap

This document captures the architectural decisions for morceau-db and the scope
...
> **Audience.** morceau-db is the OSS edge graph DB. Gigue Inc. has a private
> downstream — `phrase-db` — that wires morceau-db to its own controller stack;
> the OSS morceau-db must be usable by people who aren't Gigue. Decisions in
> this file are made through that lens: when a choice would lock morceau-db to
> Gigue's stack, the choice goes the other way and the Gigue-specific
> piece moves out of this repo.

## What morceau-db is

morceau-db is a **follower** graph store. A small embedded engine, packaged as
...
**controller** that morceau-db reaches via a generic trait; concrete
controller transports live as optional separate bridge crates.

## What morceau-db is not
...
- Not a UI / event / MCP framework. Host integrations for those concerns
  live in optional `morceau-*-hub` crates above morceau-core's surface.
...
```
  ...
  └─────────────────┼───────────────────────────────────────────┘
            ...
            │   (morceau-*-bridge)       │   e.g. morceau-surreal-bridge
            └─────────────┬────────────┘
                          │
                          ▼
                ┌────────────────────┐
...
Optional `morceau-*-hub` crates can sit between morceau-db and the host app to
provide event-driven / MCP / RAG-routing behaviour. morceau-db itself stays
...
| 3 | Controller transport | **Generic trait in morceau-core. No bundled controller transports.** Concrete bridges (e.g. `morceau-surreal-bridge`) are optional separate crates published independently. SurrealDB is Gigue's first concrete bridge target via the private `phrase-db` downstream; not bundled with morceau-db. |
| 4 | Auth | Per-user **and** per-device. morceau-db takes **opaque tokens** from the host and **opaque keys** from the controller and verifies they match — no JWT validation chain inside morceau-db. Treat all devices as potentially compromised. Identity surfaced as `user_id` + `device_id` in `morceau.toml`. |
| 5 | Conflict resolution | **Server-wins with foreshadow marking.** Local mutations carry a `foreshadow: bool` flag indicating "applied locally, not yet confirmed by controller." When the controller corrects or confirms, morceau-db flips the flag or evicts the record. CRDT-style merge is out of scope. |
...
| 8 | Schema ownership | Controller owns schema and pushes to followers over the same channel as mutations (single-channel ordering avoids the schema-late race). morceau-db does not declare. |
| 9 | Configuration | **TOML.** `morceau.toml` lives next to the host app, parsed via `serde + toml`. See `morceau.toml.example`. |
...
| 11 | Query language | **Cypher only.** Hand-rolled recursive-descent parser + interpreter. No SurrealQL or other-dialect translators inside morceau-db — those live in optional bridge / hub crates if anywhere. |
| 12 | Async runtime | **Sync `morceau-core`.** No `tokio` in core. Native uses `std::thread` for the controller worker; wasm uses `wasm-bindgen-futures` with channels. Don't bottleneck on the microtask queue — gigue's mobile model implies high inbound transaction throughput. |
| 13 | Encryption-at-rest | Out of scope through v0.0.2; storage layer must not preclude it. |
| 14 | Distribution | `morceau-core`, `morceau-wasm`, `morceau-cli` ship `publish = false` (internally usable only) through v0.0.2. Bridges and hubs may publish independently on crates.io with namespaced names (`morceau-surreal-bridge`, `morceau-supabase-bridge`, etc.). |
| 15 | License | **Dual MIT / Apache-2.0** at the top level. `cpp-reference/` retains its inherited Kuzu MIT in `cpp-reference/LICENSE-MIT-KUZU`. morceau-db copyright: Gigue Inc. and morceau-db Contributors. |
...
| 18 | OSS posture / bridges architecture | morceau-core stays controller-agnostic. **Concrete controller transports never live inside `crates/morceau-*`** — they ship as optional separate `morceau-*-bridge` crates. Likely partition: `bridges/` (controller transports, e.g. `morceau-surreal-bridge`) and `hubs/` (host-side event / MCP / RAG layers, e.g. `morceau-hub`). Crates.io publishing is namespaced (`morceau-...`). |
| 19 | Metadata as data | morceau-db's internal state — foreshadow flags, override history, capability profile, mutation log — is exposed as queryable Cypher tables, not a separate API. `MATCH (n) WHERE n._morceau.foreshadow = true RETURN n` works. Keeps the surface uniform; no parallel state-inspection vocabulary. |
| 20 | Capability profile | Top-level `[capability]` section in `morceau.toml` reports **deterministic facts** about the host — RAM MB, CPU cores, storage MB, arch, etc. morceau-db reports facts only. **No qualitative labels** ("medium", "sufficient") — host and controller decide what counts as enough. Auto-discovery is v0.0.3+. |
| 21 | Edge strategies | Both **edge-is-tiny** (morceau-db as cache + foreshadow buffer; controller does real compute) and **edge-is-free** (morceau-db executes locally based on capability profile; controller hands off opportunistically) are first-class. Configurable via `[capability]` and `[edge]`; no compile-time strategy lock-in. |
...
| **alpha.2** | Rust workspace: `crates/{morceau-core, morceau-wasm, morceau-cli}`. TOML config (`MorceauConfig`) via `serde`. Sync skeleton ported 1:1 from C++. Rust CI. Dual licensing. | ✅ shipped |
...
| **alpha.5** | WAL → MutationLog tee. `morceau-wasm` real bindings (`Morceau::open` + `query`). `morceau-cli bench` latency harness. CI runs `wasm-opt -Oz` and enforces 2 MiB budget. 45 tests. | ✅ shipped |
...
| **v0.0.3** | **Tag.** "Run on a real device" — host on iOS / Android / edge ships an app that actually persists. `tracing` (no-op default subscriber); `[capability]` auto-discovery (probe-primary on native via sysinfo / `available_parallelism` / fs2, on wasm via `navigator.deviceMemory` / `navigator.hardwareConcurrency`; declared TOML wins per-field); `morceau bench --cold-start` harness; native-only `Spawner` trait + default `StdThreadSpawner` + `Engine::with_controller_spawned_by`; real `gloo_timers`-backed wasm sleep; wasm host-storage shim (`morceau_wasm::WasmHostStorage` + `MorceauHostStorage` JS interface); `Storage` trait `MaybeSend` marker; new `morceau-ffi` crate (cdylib + rlib) with the minimum-viable C ABI shim (`morceau_version`) and CI matrix for `aarch64-apple-ios` + `aarch64-linux-android` via `cargo check`. 101 tests at tag; 791 KB / 37.8% of WASM budget. | ✅ shipped |
| **v0.0.3-alpha.1** | **Instrumentation foundation.** `tracing` crate added (no-op until host wires a subscriber), instrumented on `Engine::open_with` + the controller worker's connect / apply / retry paths. `[capability]` auto-discovery: probe is now primary (cores via `std::thread::available_parallelism`, RAM via `sysinfo`, disk via new `Storage::free_space`, arch via `cfg!`), per-field overridable from `morceau.toml`'s `[capability]` section. `morceau bench --cold-start [--seed N] [--iterations N] [--backend memory|file]` measures `Engine::open` timing. Native target decision: `cdylib` (no uniffi yet — defer until Swift FFI ergonomics force it). 93 tests (48 → 56 unit; +8 capability tests). | ✅ shipped |
...
| **v0.0.3-alpha.3** | **Wasm storage shim.** Closes the wasm-`MemoryStorage`-only gap. New `morceau_wasm::WasmHostStorage` + `MorceauHostStorage` JS interface (`append` / `readAt` / `len` / `truncate` / optional `freeSpace`) lets hosts plug in OPFS / app sandbox / RN bridge / etc. New entry point `Morceau::open_with_host_storage(toml, storage)`. **No canonical OPFS impl in morceau-wasm** — host territory, same posture as the bridges architecture. `Storage` trait gains a `MaybeSend` marker (drops the `Send` bound on wasm32 only — `JsValue` is `!Send`); native still requires `Send`. New `StorageError::JsHostError { message }` variant for host-thrown exceptions. Capability-probe wasm extension (`navigator.*`) parked for the alpha.5 audit pass. 100 tests (+3 host_storage integration). WASM 790 KB / 37.7% of 2 MiB budget. | ✅ shipped |
| **v0.0.3-alpha.4** | **Native target landing.** New `morceau-ffi` crate (`cdylib` + `rlib`) — minimum-viable C ABI shim for hosts that consume morceau-db as a dylib instead of wasm (Swift on iOS, Kotlin on Android, edge runtimes that aren't wasm hosts). Single symbol shipped (`morceau_version` returning a NUL-terminated C string); engine surface (`morceau_open` / `morceau_query` / `morceau_close`) lands in v0.0.4 alongside the crates.io publish, when the public API is stable enough to commit to a C ABI shape. Workspace `unsafe_code = "forbid"` overridden inside `morceau-ffi` only — every other crate stays forbid-unsafe. New CI matrix `native-targets` runs `cargo check` on `aarch64-apple-ios` + `aarch64-linux-android` for both `morceau-core` and `morceau-ffi` (compilation smoke; full link needs Apple's clang or Android NDK and is deferred to alpha.5 audit pass — possibly with a macOS runner). 101 tests (+1 morceau-ffi unit). | ✅ shipped |
...
| **v0.0.4** | **"Real Cypher queries at scale" — North Star: apps express queries without falling back to engine API, on graphs sized the way gigue actually uses them.** Edge queries `MATCH (a)-[r]->(b)`; multi-pattern `MATCH`; `DETACH DELETE` / cascade; property type expansion (timestamps + lists at minimum); property-type validation against schema; `ORDER BY` + simple aggregates (`count`, `collect`); edge label + property indexes (graduated from "alpha.5+ work" — gigue B2B target makes O(N) edge scans non-negotiable); ID-namespace split (nodes / edges separate); `foreshadow_eager = false` and `schema_cache = "fetch"` carry to v0.0.5 (need a real bridge to validate); cross-engine perf vs Kuzu deferred to an offline maintainer-run protocol (documented in `KUZU_DIVERGENCE.md`). **First crates.io publish** for `morceau-core` / `morceau-wasm` / `morceau-cli` / `morceau-ffi` with a pre-v0.1 "fluid API; expect breakage" README note — public-but-not-frozen API surface. `morceau-ffi` ships its minimal v0.0.3-alpha.4 surface (`morceau_version` only); engine surface defers to v0.0.5+ when the public API is stable enough to commit to a C ABI shape. Publish gated on a focused security audit of `morceau-ffi`'s `unsafe` surface (the workspace's only `forbid` exception); audit closed in alpha.5, results + future-pass checklist live in `crates/morceau-ffi/src/lib.rs` module doc. | in flight |
...
| **v0.0.4-alpha.4** | **Query polish + adjacency index + scale bench.** New `edges_by_from` + `edges_by_to` adjacency indexes; `iter_edges_from(node, label)` + `iter_edges_incident_to(node)` APIs. Closes the alpha.2 `path_candidates` and alpha.3 `delete_node_with_cascade` perf debts in one pass. Interpreter pushes `WHERE id(start) = $x` down into the start-node lookup for path patterns. Cypher polish: `ORDER BY <expr> [ASC\|DESC]`, `count(n)`, `collect(n.prop)`. `morceau bench --scale` for morceau-db's own numbers at the gigue B2B target — **10k nodes + 100k edges, indexed edge MATCH p50 = 2.67µs (10,000× over pre-alpha.4 path_candidates).** Scope trims (all documented in LIMITATIONS): `foreshadow_eager = false` + `schema_cache = "fetch"` carry forward to v0.0.5 (need a real bridge to validate); cross-engine perf benchmark vs upstream Kuzu moves to v0.0.4-alpha.5 audit pass (Kuzu install + harness is alpha-sized work on its own). 148 tests (+9 query_polish integration). | ✅ shipped |
| **v0.0.4-alpha.5** | **Audit pass + first crates.io publish gate.** `morceau-ffi` `unsafe` audit cleared (single symbol `morceau_version`; no pointer params; no `unsafe impl Send/Sync`; C ABI is `*const c_char` only; workspace `forbid` lint intact on every other crate; audit results recorded inline in `crates/morceau-ffi/src/lib.rs` module doc and footer in `LIMITATIONS.md`). Workspace `publish = false` → `true` flipped. `morceau-ffi` ships **minimal** at v0.0.4 (`morceau_version` only); engine surface (`morceau_open` / `morceau_query` / `morceau_close`) deferred to v0.0.5+ when the public API is stable enough to commit to a C ABI shape — keeps the audit small and gets the crates.io presence earlier. Cross-engine perf benchmark vs Kuzu deferred to an offline maintainer-run protocol (Kuzu prebuilt binary install + equivalent Person/KNOWS fixture; protocol + TBD numbers table in `KUZU_DIVERGENCE.md`). Docs sweep: README pre-v0.1 + Install section + morceau-ffi in workspace listing; `morceau-core/src/lib.rs` module doc refreshed from stale "v0.0.2-alpha.2" to current Engine / Schema / Cypher surface; LIMITATIONS retired/accepted block for alpha.5; MORCEAU.md namechange pencil-in (see Open questions). 148 tests (no functional changes; audit-only pass). | ✅ shipped |
| **v0.0.4-alpha.6** | **Rename: `motif` → `morceau-db`.** Workspace-wide sed of every workspace token, public identifier, on-disk magic, Cypher metadata namespace, file path, and doc reference. The Open-question item 1 penciled in alpha.5 resolved here, before v0.1.0 freezes crate names on crates.io. Crate prefix `morceau-`; brand `morceau-db`; binary `morceau`; org / flagship repo `morceau-db/morceau-db`. Per-layer: filesystem (`crates/motif-*` → `crates/morceau-*`, `MOTIF.md` → `MORCEAU.md`, `motif.toml.example` → `morceau.toml.example` — all via `git mv` so history is preserved at 63–100% per-file similarity); workspace + per-crate manifests (`name = "morceau-*"`, binary `name = "morceau"`, `repository = "https://github.com/morceau-db/morceau-db"`); on-disk wire identity (`MAGIC: b"MOTIF\0\0\x01"` → `b"MORCEAU\x01"`, still 8 bytes, `FORMAT_VERSION` unchanged at 4; Cypher metadata `_motif.X` → `_morceau.X`); Rust public surface (`Motif` → `Morceau`, `MotifConfig` → `MorceauConfig`, `MotifHostStorage` → `MorceauHostStorage`, C ABI `motif_version` → `morceau_version`); CI workflow `-p motif-*` → `-p morceau-*`. cpp-reference/ left untouched (frozen historical tree). 148 tests (no functional changes); 850 KB / 40.6% of 2 MiB after wasm-opt (vs alpha.5 851 KB). | ✅ shipped |
| **v0.0.4-alpha.7** | **OSS-reputability scaffolding.** Placeholder shells for the v0.0.4 publish-readiness checklist so the project structure looks reputable to crates.io consumers + downstream auditors when the curtain pulls back. New (placeholder): `SECURITY.md`, `CONTRIBUTING.md`, `CHANGELOG.md`, `deny.toml`, `.github/dependabot.yml`, `.github/workflows/cargo-deny.yml`. `CODE_OF_CONDUCT.md` updated by maintainer in the same merge window. Content fill-in (real disclosure address / vuln-reporting channel; CHANGELOG digest per release; cargo-deny policy; dependabot schedules; contributor-protocol prose) is on the maintainer's timeline. README badges + MORCEAU.md milestone-row backfill (this row + alpha.6) deferred to a follow-up. Out of scope: GitHub org / repo transfer (user-driven); `cargo publish` (release PR is its own thing). | ✅ shipped |
| **v0.0.5** | **"Hostile-device-aware" — North Star: auth + encryption are real, host policy is honoured, no quiet escape hatches.** New `[security]` TOML section: `require_authenticated_channel`, `min_aead`, `pq_required`, suite allow-list. Bridges advertise their crypto suite at handshake; morceau-db validates declared-vs-policy and surfaces `ControllerSecurityError` on mismatch. Opt-out is explicitly named `Engine::dangerously_*` (Rust convention). SOTA-classical allow-list (X25519, ChaCha20-Poly1305, AES-GCM) is B2B table stakes. **PQ forward-compat plumbing** — Signal-PQXDH-style scanner, fail-visible when host requires PQ but bridge doesn't advertise; PQ implementation itself is stretch. Host token-issuance and controller key-rotation flows documented + tested. Encryption-at-rest design + impl. CRC on records. Tee panic-safety test. | planned |
...
| **v0.1.0** | **"OSS-ready" — North Star: external contributors `cargo add morceau-core` with semver promises; first concrete bridge ships independently.** Public API freeze + audit; semver policy documented; first concrete bridge crate (the experience answers the `bridges/` vs `hubs/` partition question); bridge-author + hub-author guides; documentation pass (API docs, getting-started, deployment); `cpp-reference/` archived to a separate tag / repo. | planned |

## Exit criteria for v0.0.1

All satisfied at tag (see PR #1):

1. `cargo build --release --target wasm32-unknown-unknown -p morceau-wasm` produces a loadable module; `cargo run -p morceau-cli -- print-config morceau.toml.example` round-trips.
2. `Morceau::open` API works native + wasm.
3. TOML schema documented; `serde` rejects malformed configs.
4. `morceau-db.query("MATCH (n) RETURN n LIMIT 10")` returns rows.
...
3. **Controller trait fully abstract.** No SurrealDB / network-DB / Surreal-anything in any `crates/morceau-*`. `cargo tree -p morceau-core` shows no network deps.
...
8. **No `unsafe` in `morceau-core`** preserved.
9. **`[capability]` documented** with `default` / `potato` / `hoverphone` example configs in `morceau.toml.example`. All values are deterministic facts (numbers / enums) — no qualitative labels.
10. **Metadata-as-data demonstrated**: `MATCH (n) WHERE n._morceau.foreshadow = true RETURN n` returns the right set; same for override history.
...
`morceau-surreal-bridge` (Gigue's job; lands separately). `morceau-hub` / `morceau-mcp` host-event layers (optional, separate). Any non-Cypher dialect support. Encryption-at-rest. Multi-tenant. Vector / FTS. Native targets (`aarch64-apple-ios`, `aarch64-linux-android`). Real JWT validation in morceau-db (host's job). Cypher surface beyond v0.0.1 (no `ORDER BY`, no aggregates, no multi-pattern). Conflict-resolution beyond server-wins (no CRDTs, no OT). Public crates.io publish for `morceau-core` / `morceau-wasm` / `morceau-cli`. Auto-discovery of `[capability]`. iOS/Android-specific bench profiles.
...
morceau-db is designed for a **polypartite graph in B2B collaboration
...
- Crates.io publish for `morceau-core` / `morceau-wasm` / `morceau-cli` /
  `morceau-ffi`.
...
- Public-but-not-frozen API surface. v0.1.0 freezes; v0.0.x can
  break.
- **Publish gate: focused security audit of `morceau-ffi`'s `unsafe`
  surface.** v0.0.3-alpha.4 shipped the crate with a single symbol
  (`morceau_version`, no real `unsafe`); v0.0.4-alpha.5 audits that
  minimal surface and ships it as-is to crates.io. Engine surface
  (`morceau_open` / `morceau_query` / `morceau_close`) with pointer-taking
  ...
  layout. Checklist + audit results live in `crates/morceau-ffi/src/lib.rs`
  module doc. morceau-ffi is the workspace's only `unsafe_code = "forbid"`
  relaxer — it deserves the clear line of sight before strangers start
  linking against it.

### v0.0.5 — "Hostile-device-aware"

Auth + encryption become real. morceau-db imposes nothing — it inherits
...
- Bridges advertise their crypto suite at `Controller::connect`.
  morceau-db validates declared-vs-policy.
...
- **PQ forward-compat plumbing** — Signal-PQXDH-style scanner,
  ...
  upgrade, not a morceau-db upgrade.
...
External contributors `cargo add morceau-core` with semver promises.
...
- **Cypher beyond "query layer".** No transactions, no procedural
  ...
  that surface translate before talking to morceau-db.

## Open questions

1. **Project name evaluation — RESOLVED in v0.0.4-alpha.6.**
   Penciled in during the v0.0.4-alpha.5 audit pass; the original
   project name "motif" collided with a preponderance of
   "motif"-named databases and library tools in the wider market,
   especially in biostatistics / bioinformatics (sequence motif
   discovery), where "motif DB" is an entrenched term of art. The
   collision was discoverability cost, not legal — but it compounded
   the closer the project got to v0.1.0 (API freeze + first external
   bridge consumers + first crates.io anchor versions). v0.0.4-alpha.6
   sed-renamed every workspace token, identifier, on-disk magic,
   Cypher metadata namespace, file path, and doc reference from
   `motif` to `morceau` (workspace token / crate prefix / binary /
   file paths) or `morceau-db` (brand for user-facing prose; org /
   flagship repo name). Item left here for historical context until
   v0.1.0 archives this document.
...
3. **`bridges/` vs `hubs/` partition.** First bridge and first hub
   ...
   whether everything wants to be `morceau-*-ext/` or similar. Waits
...
morceau-db/
  ...
  morceau.toml.example               # canonical config schema (with [capability])
  crates/
    morceau-core/                    # engine library, sync core, no_unsafe
    morceau-wasm/                    # wasm-bindgen surface (cdylib)
    morceau-ffi/                     # C ABI shim for iOS/Android (cdylib + rlib;
                                   # only crate that allows unsafe — local override)
    morceau-cli/                     # dev/smoke CLI + bench
  ...
  MORCEAU.md, README.md, LIMITATIONS.md
```
...
Inc. and morceau-db Contributors) informed by the C++ baseline as a behavioural
