...
| **v0.0.3** | **Tag.** "Run on a real device" — host on iOS / Android / edge ships an app that actually persists. `tracing` (no-op default subscriber); `[capability]` auto-discovery (probe-primary on native via sysinfo / `available_parallelism` / fs2, on wasm via `navigator.deviceMemory` / `navigator.hardwareConcurrency`; declared TOML wins per-field); `motif bench --cold-start` harness; native-only `Spawner` trait + default `StdThreadSpawner` + `Engine::with_controller_spawned_by`; real `gloo_timers`-backed wasm sleep; wasm host-storage shim (`motif_wasm::WasmHostStorage` + `MotifHostStorage` JS interface); `Storage` trait `MaybeSend` marker; new `motif-ffi` crate (cdylib + rlib) with the minimum-viable C ABI shim (`motif_version`) and CI matrix for `aarch64-apple-ios` + `aarch64-linux-android` via `cargo check`. 101 tests at tag; 791 KB / 37.8% of WASM budget. | ✅ shipped |
...
motif/
  ...
  crates/
    ...
    motif-ffi/                     # C ABI shim for iOS/Android (cdylib + rlib;
