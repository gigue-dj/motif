...
| **v0.0.3** | **"Run on a real device" — North Star: a host on iOS / Android / edge ships an app that actually persists, with measurable cold-start.** Wasm storage shim (OPFS / app sandbox / wasm-bindgen-driven I/O); `Spawner` trait so hosts opt into platform runtimes (iOS GCD, Android coroutines); native `cdylib` evaluation for `aarch64-apple-ios` + `aarch64-linux-android` (`cdylib` chosen in alpha.1 per the long-run strategy answer); wasm sleep via `gloo-timers`; cold-start measurement harness; `tracing` crate with no-op default subscriber; `[capability]` auto-discovery (native + wasm). | ready |
