...
| **0.0.2-alpha.5** | **Audit pass.** PR #1 `[debt]` items + v0.0.2 debt closed or knowingly accepted. `Engine::with_named_controller` for opt-in `kind` validation. New `KUZU_DIVERGENCE.md` at the repo root cataloguing what we cut from upstream Kuzu (storage, query, concurrency, indexes, types, catalog) so contributors who arrive expecting Kuzu can see the deltas at a glance. Bench harness extended for file-backed and thread-contention cases. | pending |
| **v0.0.2** | Tag. | pending |
