...
| **alpha.4** | Hand-rolled lexer + recursive-descent parser + direct interpreter for the Cypher subset documented in `query/mod.rs`: `CREATE`, `MATCH`/`WHERE`/`RETURN`/`LIMIT`, `MERGE`, `MATCH ... DELETE`. Single-variable patterns, label + property predicates, the `id(n)` builtin, the fast-path id-lookup in the interpreter. Engine extended with `delete_node` / `delete_edge` / `iter_nodes` / `iter_edges` and a `Record::*Delete` variant. ~1,700 LOC including tests (over the original 800 LOC estimate — the lexer alone is 473 LOC after `rustfmt` line breaks). 33 unit tests + 8 integration tests, no `unsafe`. | ✅ shipped |
| **alpha.5** | WAL → `MutationLog` tee in `Engine` (architectural validation: every committed insert / delete produces a `Mutation` stamped with the per-instance `ActorId`, before the index goes live for inserts). `motif-wasm` real bindings: `Motif::open(toml_src)` + `Motif::query(cypher, params_json) -> result_json` + `mutation_count()`, params and result marshalled as JSON over the wasm ABI. `motif-cli bench` latency harness — id-lookup p50 / p95 / p99 with a hard PASS/FAIL against the 50 ms exit criterion. CI installs `binaryen` and runs `wasm-opt -Oz`, then enforces the 2 MB size budget. | ✅ shipped |
